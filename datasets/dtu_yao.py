...
import random
from torchvision import transforms  # 新增：引入 torchvision 的 transforms 模块
...
class MVSDataset(Dataset):
    def __init__(self, datapath, listfile, mode, nviews, ndepths=192, interval_scale=1.06, **kwargs):
        ...
        # # 新增：定义色彩抖动变换（仅在训练时使用）
        # self.color_augment = transforms.ColorJitter(brightness=0.5, contrast=0.5, saturation=0.5, hue=0.1)
    ...
    def __getitem__(self, idx):
        ...
        if self.mode == "train":
            ...
            if len(src_views) >= self.nviews - 1:
                ...
                src_views_ids = src_views
        else:
            # 非训练模式下，固定选择前 nviews-1 个视图
        ...
        for i, vid in enumerate(view_ids):
            ...
            img = self.read_img(img_filename) # read_img 中包含了色彩抖动
