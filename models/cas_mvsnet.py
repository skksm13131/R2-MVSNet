...
class DepthNet(nn.Module):
    ...
    def forward(self, features, proj_matrices, depth_values, num_depth, cost_regularization,
        ...
        if view_attention is None:
            ...
            for src_fea, src_proj, src_feature_reliability in zip(
                    src_features, src_projs, src_feature_reliabilities):
                src_proj_new = self._compose_proj(src_proj)
                warped_volume = homo_warping(src_fea, src_proj_new, ref_proj_new, depth_values)
...
class CascadeMVSNet(nn.Module):
    def __init__(self, refine=False, ndepths=[48, 32, 8], depth_interals_ratio=[4, 2, 1], share_cr=False,
                 ...
                 view_attention_mode='single_pass_reliability_weighted', use_rafe=False,
        ...
        if self.use_view_attention and self.view_attention_mode != 'single_pass_reliability_weighted':
            raise ValueError('only the SP-RWCV view-attention mode is retained')
        print('**********netphs:{}, depth_intervals_ratio:{}, grad:{}, chs:{}, sp_rwcv:{}, rafe:{}, fgdr:{}, fgdr_anchor_base:{}************'.format(
              ndepths, depth_interals_ratio, self.grad_method, self.cr_base_chs,
              use_view_attention, use_rafe, use_fgdr, fgdr_anchor_base))
